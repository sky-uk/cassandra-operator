	...
	Describe("event dispatching", func() {
		...
		Context("cluster changes", func() {
			...
			It("should dispatch an AddCluster event when no corresponding cluster found", func() {
				fakes.noConfigMapToReconcile(desiredCassandra, configMapNamespaceName)
				fakes.cassandraIsFoundIn(clusterNamespaceName, desiredCassandra)
				fakes.errorWhenLookingForCurrentStateFor(desiredCassandra)
		...
		Context("configMap changes", func() {
			...
			// Ignored until we confirm either manually or better still via an e2e test that this is the behaviour we want
			PIt("should dispatch an AddCustomConfig event when some statefulsets do not have a configMap??", func() {
				desiredCassandra.Spec.Racks = []v1alpha1.Rack{
					{Name: "a", Replicas: 1, StorageClass: "rack-storage-a", Zone: "rack-zone"},
					{Name: "b", Replicas: 1, StorageClass: "rack-storage-b", Zone: "rack-zone"},
				}
		...
		Context("configMap and Cassandra changes", func() {
			var (
				configMap *corev1.ConfigMap
			)
			...
			It("should dispatch configMap dispatchedEvents before Cassandra ones", func() {
				currentCassandra := desiredCassandra.DeepCopy()
				currentCassandra.Spec.Pod.CPU = resource.MustParse("101m")
				fakes.cassandraIsFoundIn(clusterNamespaceName, desiredCassandra)
				fakes.currentCassandraStateIsFoundFor(desiredCassandra, currentCassandra)
...
func (m mocks) noConfigMapToReconcile(desiredCassandra *v1alpha1.Cassandra, configMapNamespaceName types.NamespacedName) {
...
func (m mocks) errorWhenLookingForCurrentStateFor(desiredCassandra *v1alpha1.Cassandra) {
